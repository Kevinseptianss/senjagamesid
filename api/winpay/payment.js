...
const setCorsHeaders = (res) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Timestamp, X-Partner-ID, X-Signature, X-External-ID, Channel-ID');
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (clientSecret && clientSecret !== 'your_secret_here') {
      const isValidSignature = verifyWinPaySignature(req.body, timestamp, partnerId, signature, clientSecret);
      if (!isValidSignature) {
    ...
    // Verify partner ID
    const expectedPartnerId = process.env.WINPAY_PARTNER_ID || 'fe515458-df5e-4ab6-9136-84b18e79f1e8';
