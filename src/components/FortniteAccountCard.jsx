    ...
    >
      ...
      <div className="p-4 space-y-3 flex-1">
        ...
        <div className={`flex items-center space-x-2 p-2 rounded-lg border ${
          lastSeenClass === 'warn-green' ? 'bg-green-900 bg-opacity-30 border-green-700' :
          lastSeenClass === 'warn-yellow' ? 'bg-yellow-900 bg-opacity-30 border-yellow-700' :
          lastSeenClass === 'warn-orange' ? 'bg-orange-900 bg-opacity-30 border-orange-700' :
          'bg-red-900 bg-opacity-30 border-red-700'
        }`}>
          <Icon icon="mdi:clock-outline" className={`text-sm ${
            lastSeenClass === 'warn-green' ? 'text-green-400' :
            lastSeenClass === 'warn-yellow' ? 'text-yellow-400' :
            lastSeenClass === 'warn-orange' ? 'text-orange-400' :
            'text-red-400'
          }`} />
          <span className={`text-xs font-medium ${
            ...
            Last seen {account.lastSeen || account.last_seen || 
                      (lastSeenClass === 'warn-green' ? 'Recently' :
                       lastSeenClass === 'warn-yellow' ? 'This week' :
                       lastSeenClass === 'warn-orange' ? 'This month' : 'Long ago')}
          </span>
        </div>
            ...
            {topCosmetics.map((cosmetic, index) => {
              ...
              <div key={index} className="flex items-center bg-gray-800 p-2 rounded-lg border border-gray-600 hover:border-gray-500 transition-colors">
                <div className="w-6 h-6 rounded mr-3 flex-shrink-0 flex items-center justify-center bg-gray-700">
                  ...
                  <div className="w-6 h-6 bg-gray-700 rounded flex items-center justify-center text-gray-400 text-sm font-bold" style={{display: cosmetic.image ? 'none' : 'flex'}}>
                    {cosmetic.type === 'Skin' ? '👤' : 
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {cosmetic.rarity && (
                      <span className={`text-xs px-1 py-0.5 rounded text-white ${
                        cosmetic.rarity.toLowerCase() === 'legendary' ? 'bg-orange-600' :
                        cosmetic.rarity.toLowerCase() === 'epic' ? 'bg-purple-600' :
                        cosmetic.rarity.toLowerCase() === 'rare' ? 'bg-blue-600' :
                        cosmetic.rarity.toLowerCase() === 'uncommon' ? 'bg-green-600' :
                        'bg-gray-600'
                      }`}>
                        {cosmetic.rarity.charAt(0).toUpperCase() + cosmetic.rarity.slice(1).toLowerCase()}
                      </span>
                    )}
      ...
      <div className="bg-gray-800 px-4 py-3 border-t border-gray-700 flex items-center justify-between text-xs mt-auto">
        ...
        <div className="flex items-center space-x-2 text-gray-400">
          <time className="hover:text-gray-300">
            {account.fortnite_country || account.country || 'Global'}
          </time>
          <span>•</span>
          <span className="capitalize-first text-gray-300 hover:text-white">
            {account.item_origin || account.origin || 
             (parseFloat(account.price || 0) > 5 ? 'Premium' : 'Standard')}
          </span>
        </div>
      </div>
