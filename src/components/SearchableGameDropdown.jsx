...
const SearchableGameDropdown = ({ 
  ...
  const availableGames = gamesList.length > 0 ? gamesList : defaultGamesList;

  // Efficient search with smart filtering and limit to 30 results
  const getFilteredGames = () => {
    if (!searchTerm.trim()) {
      // Show first 30 games when no search term (popular games will be first due to API sorting)
      return availableGames.slice(0, 30);
    }
    ...
    for (const game of availableGames) {
      if (!game.label || typeof game.label !== 'string') continue;
      ...
      if (exactMatches.length + startMatches.length + containsMatches.length >= 50) {
        break;
      }
    }
    
    // Combine results with priority: exact > starts with > contains
    // Within each category, popular games (isPopular) will already be sorted first from API
    const allMatches = [...exactMatches, ...startMatches, ...containsMatches];
    
    // Further prioritize popular games in search results
    ...
    return sortedMatches.slice(0, 30);
  };
    ...
    <div className="space-y-2" ref={dropdownRef}>
                ...
                <>
                  ...
                  {searchTerm && (
                    <div className="px-3 py-1 text-xs text-gray-400 bg-gray-750 border-b border-gray-600">
                      {filteredGames.length} hasil {filteredGames.length >= 30 ? '(maksimal 30 ditampilkan)' : ''}
                    </div>
                  )}
      ...
      {selectedGameLabels.length > 0 && (
        <div className="text-xs text-gray-400">
          {selectedGameLabels.length} game dipilih
        </div>
      )}
    </div>
  );
