...
const SteamPage = ({ onBack }) => {
  ...
  const transformSteamAccount = (apiAccount) => {
    ...
    return {
      ...
      lastSeen: formatLastSeen(steamData.last_seen || steamData.last_online || steamData.last_logoff),
  ...
  // Fetch Steam accounts with filters
  const fetchSteamAccounts = async (newFilters = filters) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      console.error('Error fetching Steam accounts:', err)
  ...
  const handleFilterChange = (newFilters) => {
    ...
    fetchSteamAccounts(updatedFilters)
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="text-gray-400 text-sm">
              Browse and filter Steam accounts
            </div>
          </div>
        </div>
      </header>
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={`font-medium ${
                        ...
                        account.status === 'Offline' ? 'text-gray-400' :
                        account.status === 'Away' ? 'text-yellow-400' :
                        'text-gray-200'
                      }`}>
                        {account.status}
                      </span>
