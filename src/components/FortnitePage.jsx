    ...
    try {
      ...
      if (newFilters.platform) {
        filteredAccounts = filteredAccounts.filter(account => 
          account.platform === newFilters.platform
        );
      }
      
      if (newFilters.lmin) {
        filteredAccounts = filteredAccounts.filter(account => 
          account.level >= parseInt(newFilters.lmin)
        );
      }
      
      if (newFilters.lmax) {
        filteredAccounts = filteredAccounts.filter(account => 
          account.level <= parseInt(newFilters.lmax)
        );
      }
      
      if (newFilters.vbmin) {
        filteredAccounts = filteredAccounts.filter(account => 
          account.vbucks >= parseInt(newFilters.vbmin)
        );
      }
      
      if (newFilters.vbmax) {
        filteredAccounts = filteredAccounts.filter(account => 
          account.vbucks <= parseInt(newFilters.vbmax)
        );
      }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {accounts.map((account) => (
                <div key={account.id} className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden hover:border-purple-500 transition-colors">
                  {/* Account Image */}
