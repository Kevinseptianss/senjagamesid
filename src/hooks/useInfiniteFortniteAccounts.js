...
export const useInfiniteFortniteAccounts = (filters = {}) => {
  ...
  const transformAccountData = (apiAccount) => {
    ...
    return {
      ...
      priceWithSellerFeeLabel: apiAccount.priceWithSellerFeeLabel || (apiAccount.price_currency ? 
      ...
      pickaxesCount: Array.isArray(fortniteData.fortnite_pickaxes || fortniteData.pickaxes) ? 
        (fortniteData.fortnite_pickaxes || fortniteData.pickaxes).length : 0,
      emotesCount: Array.isArray(fortniteData.fortnite_emotes || fortniteData.emotes) ? 
        (fortniteData.fortnite_emotes || fortniteData.emotes).length : 0,
      glidersCount: Array.isArray(fortniteData.fortnite_gliders || fortniteData.gliders) ? 
        (fortniteData.fortnite_gliders || fortniteData.gliders).length : 0,
      ...
      // Helper properties
      totalCosmetics: (
        (Array.isArray(fortniteData.fortnite_skins || fortniteData.skins) ? (fortniteData.fortnite_skins || fortniteData.skins).length : 0) +
        (Array.isArray(fortniteData.fortnite_pickaxes || fortniteData.pickaxes) ? (fortniteData.fortnite_pickaxes || fortniteData.pickaxes).length : 0) +
        (Array.isArray(fortniteData.fortnite_emotes || fortniteData.emotes) ? (fortniteData.fortnite_emotes || fortniteData.emotes).length : 0) +
        (Array.isArray(fortniteData.fortnite_gliders || fortniteData.gliders) ? (fortniteData.fortnite_gliders || fortniteData.gliders).length : 0)
      ),
  ...
  const loadInitialAccounts = useCallback(async () => {
    ...
    try {
      ...
      const fortniteParams = Object.keys(transformedFilters).length === 0 ? 
        defaultFortniteFilters : 
        { ...defaultFortniteFilters, ...transformedFilters, page: 1 };
      
      const response = await api.getFortniteAccounts(fortniteParams);
      ...
      if (response && response.items) {
        ...
        const actualTotal = response.totalCount || response.total || 0;
        const actualPerPage = response.itemsPerPage || response.per_page || fortniteParams.per_page || 25;
        const totalPages = Math.ceil(actualTotal / actualPerPage);
        const hasMorePages = totalPages > 1 && response.items.length >= actualPerPage;
  ...
  const loadMoreAccounts = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.items && response.items.length > 0) {
        ...
        const actualPerPage = response.itemsPerPage || response.per_page || fortniteParams.per_page || 25;
        const totalPages = Math.ceil(actualTotal / actualPerPage);
        const hasMorePages = nextPage < totalPages && response.items.length >= actualPerPage;
...
const generateDemoFortniteAccounts = () => {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    account.totalCosmetics = account.skinsCount + account.pickaxesCount + account.emotesCount + account.glidersCount;
